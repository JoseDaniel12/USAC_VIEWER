...
public class ListaImagenes extends EstructuraDeDatos {
    ...
    public Object find(Object e) {
        ...
        if (index > 0) {
           for (int i = 0; i < index; i++) {
               if (aux.getValue().icon == icon) {
                   return aux.getNext().getValue().icon;
               }
               aux = aux.getNext();
           }        
        }  
    ...
    public Object getPrevious(ImageIcon icon) {
        ...
        if (index > 0) {
           for (int i = 0; i < index; i++) {
               if (aux.getNext().getValue().icon == icon) {
                   return aux.getValue().icon;
               }
               aux = aux.getNext();
           }        
        }  
    ...
    public void delete(Object e) {
        ...
        NodoImagen aux = cabeza; 
        if (find(icon) != null) {
            if (cabeza.getValue().icon == icon) {
                cabeza = cabeza.getNext();
                cabeza.setPrevious(end);
                end.setNext(cabeza);
            } else if (end.getValue().icon == icon) {
                end.getPrevious().setNext(cabeza);
            } else {
                while(aux.getNext().getValue().icon != icon) {
                    aux = aux.getNext();
                }
